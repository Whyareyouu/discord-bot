...
async function paginate(interaction, elements, timeout = 60_000, callback) {
    ...
    const generateEmbed = (page) => {
        ...
        const description = elements.slice(start, end)
            .map((element, index) => `${start + index + 1}. **${element.title}**\n${element.description}`)
            .join('\n\n');
    ...
    const navigationRow = new ActionRowBuilder().addComponents(prevPageButton, nextPageButton);
    ...
    const filter = (i) => {
        return i.customId.startsWith('item_') || (i.customId === 'prev_page' || i.customId === 'next_page') && i.user.id === interaction.user.id
    }
    ...
    collector.on("collect", async (confirmation) => {
        try {
            if (confirmation.customId === 'prev_page' && currentPage > 0) {
                currentPage--;
            } else if (confirmation.customId === 'next_page' && currentPage < totalPages - 1) {
                currentPage++;
