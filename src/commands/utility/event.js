...
module.exports = {
    ...
    async execute(interaction) {
        const embed = new EmbedBuilder()
            ...
            .setImage('https://media1.tenor.com/m/6jzIXECrncsAAAAC/goosegooseduck-ggd.gif')
        ...
        const joinActionRow = new ActionRowBuilder()
            .addComponents(join_event);

        const message = await interaction.reply({embeds: [embed], components: [joinActionRow]});
        ...
        collector.on('collect', async (confirmation) => {
            ...
            if (confirmation.customId === 'join') {
                await Events.update({'players': sequelize.fn('array_append', sequelize.col('players'), userId)}, {where: {id: event.id}})
                const playersCount = await Events.findByPk(event.id).then(event => event?.dataValues?.players?.length)
                ...
                const leaveActionRow = new ActionRowBuilder()
                    .addComponents(leave_event);
                ...
                await Events.update({'players': sequelize.fn('array_remove', sequelize.col('players'), userId)}, {where: {id: event.id}})
        ...
        collector.on('end', async () => {
            ...
            const endActionRow = new ActionRowBuilder()
                .addComponents(endEvent);

            await message.edit({ embeds: [embed], components: [endActionRow] });

            const acceptedUsers = await Events.findByPk(event.id)

                acceptedUsers?.dataValues?.players?.forEach(async (userId) =>await message.client.users.send(userId, "Вы приняли участвие в событии"))
        });
